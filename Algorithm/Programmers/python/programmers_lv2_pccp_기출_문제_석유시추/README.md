...
- 열 -> 행 탐색
  - 하나의 열에서 행 별로 탐색함
- 탐색 과정에서 석유 매장 영역의 입구에 도달하면 그 지점을 해당 영역의 head 노드로 선정
- 매장 영역의 석유량은 head 노드로 기록
- 다음 열을 탐색할 때, head노드가 이미 기록 되어 있는 영역이라면 이전에 기록한 head 노드의 석유량 값을 더하면서 해당 열에서 시추 가능한 총 석유량을 기록
